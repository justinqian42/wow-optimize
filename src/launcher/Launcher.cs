    ...
    public class MainForm : Form {
        ...
        public MainForm() {
            ...
            settingsMap = new Dictionary<string, SettingItem>() {
                ...
                { "SSE2 Network GUID Unpacking", new SettingItem("Combat_Net", "NetworkGuidSse2", true, null, "Vectorizes the unpacking of network entity GUIDs inside network data streams.") },
                ...
                { "Bone Matrix Upload (SSE2)", new SettingItem("Graphics_Sound", "BoneMatrixUpload", true, null, "Replaces the bone matrix transpose in the draw path with SSE2. 3.35% of main-thread time in the profile. Nothing is computed, only copied, so the result is identical bit for bit; it still checks the first 20000 bones against the client and backs out if they differ.", true) },
                ...
                { "M2 Batch Matrix Setup (SSE2)", new SettingItem("Graphics_Sound", "M2BatchMatrixSse2", true, null, "Replaces the serialized sixteen-float matrix setup copies in sub_823130 (M2 batch render pass setup) with four SSE2 vector moves each. Eliminates x87 store-forwarding stalls during model rendering.", true) },
