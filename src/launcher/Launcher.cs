    ...
    public class MainForm : Form {
        ...
        public MainForm() {
            ...
            settingsMap = new Dictionary<string, SettingItem>() {
                ...
                { "Critical Section Hook (All Modules)", new SettingItem("General", "LockTuningInitHook", true, null, "Hooks InitializeCriticalSection in ntdll so every lock created after startup gets a spin count. Every module in the process goes through that hook, overlays and ReShade included, and No Client Patches does not stop it because it is outside WoW.exe. A player with ReShade could not enter the world until Critical Section Spin Tuning was off; if that is you, turn this one off first and say whether it was enough. Inherits Critical Section Spin Tuning when its own key is absent.") },
        ...
        private void ApplyInheritedDefaults(Dictionary<string, string> present) {
            ...
            InheritIfAbsent(present, "LockTuningInitHook", "LockTuning");
