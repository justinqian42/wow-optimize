// ============================================================================
// Module: collision_bsp_traverse_sse2.cpp
//
// Iterative world collision BSP tree traversal (sub_7CA920).
//
// In world collision detection and raycasting (sub_7CB7B0 -> sub_7CA920),
// sub_7CA920 was sampled 98,447 times at 0x007CA939 as the primary traversal
// hotspot across terrain and world collision trees.
//
// Bottlenecks in the client implementation (0x007CA920 -> 0x007CAACF, 176 bytes):
//   1. Recursive function overhead: Allocates 76 bytes of stack frame per node
//      plus 3 register saves (88 bytes total frame per depth level). On trees
//      16 to 24 levels deep, this constantly creates and destroys deep call frames.
//   2. Serialized x87 status-word transfers: Each internal node executes up to
//      four separate x87 comparisons (fnstsw ax / test ah, 41h / test ah, 5) to
//      evaluate bounding box overlaps and split plane classification, stalling
...
//     1,000,000 cases with 0 mismatches in leaf visitation order (harness only,
//     not run in a game).
//   - Runs 6 fixed test vectors through a synthetic tree on startup, asserting
//     bit-exact leaf sequence and count before hooking.
//   - Off by default under experimental launcher switch CollisionBspTraverse.
// ============================================================================
...
namespace CollisionBspTraverse {

namespace {

constexpr uintptr_t kTarget      = 0x007CA920;
constexpr uintptr_t kLeafHandler = 0x007CA8C0;

// Prologue bytes of sub_7CA920 (16 bytes):
// 55:          push ebp
// 8B EC:       mov ebp, esp
...
typedef void (__fastcall* TraverseBSP_fn)(void* this_ptr, void* dummy_edx,
...
TraverseBSP_fn orig_TraverseBSP = nullptr;
...
void Fast_TraverseBSP(void* this_ptr, uint32_t root_node_id, const float* root_query_box, const float* root_node_box) {
            ...
            if (split < query_min) {
                ...
                if (left != 0xFFFF) {
                    if (top < 63) {
                        ...
                        l_qbox[axis + 3] = split;
                        l_nbox[axis + 3] = split;
                        orig_TraverseBSP(this_ptr, nullptr, left, l_qbox, l_nbox);
                    }
                }
...
__declspec(safebuffers)
void __fastcall Hooked_TraverseBSP(void* this_ptr, void* dummy_edx,
                                   uint32_t node_id, const float* query_box, const float* node_box) {
    if (!this_ptr || !query_box || !node_box) return;

    if (g_dead) {
        orig_TraverseBSP(this_ptr, dummy_edx, node_id, query_box, node_box);
        return;
    }

    g_calls++;
    Fast_TraverseBSP(this_ptr, node_id, query_box, node_box);
}
...
bool Init() {
    if (!Config::g_settings.OptCollisionBspTraverse) {
        return false;
    }

    if (Config::g_settings.OptNoClientPatches) {
        Log("[CollisionBspTraverse] Client binary patches disabled by OptNoClientPatches, skipping.");
        return false;
    }
    ...
    // Verify prologue bytes at sub_7CA920
    if (std::memcmp((const void*)kTarget, kExpectedPrologue, sizeof(kExpectedPrologue)) != 0) {
        Log("[CollisionBspTraverse] Refusing to hook 0x%08X: prologue bytes mismatch", kTarget);
        return false;
    }

    // Run startup self-test
    if (!RunSelfTest()) {
        Log("[CollisionBspTraverse] Refusing to hook 0x%08X: self-test failed", kTarget);
        return false;
    }

    MH_STATUS status = WineSafe_CreateHook((void*)kTarget, (void*)Hooked_TraverseBSP, (void**)&orig_TraverseBSP);
    if (status != MH_OK) {
        Log("[CollisionBspTraverse] WineSafe_CreateHook failed with %d", status);
        return false;
    }

    status = WO_EnableHook((void*)kTarget);
    if (status != MH_OK) {
        Log("[CollisionBspTraverse] WO_EnableHook failed with %d", status);
        return false;
    }

    g_installed = true;
    g_abSubject = AbTest::IsSubject("CollisionBspTraverse", &g_abSubject);
    g_benchId = SelfBench::Register("CollisionBspTraverse");

    SamplingProfiler::RegisterSelfSymbol("CollisionBspTraverse", (const void*)&Hooked_TraverseBSP);

    Log("[CollisionBspTraverse] ACTIVE on sub_7CA920 (iterative BSP tree traversal, 176 bytes, off by default).");
    return true;
}
...
void Shutdown() {
    ...
    g_installed = false;
    Log("[CollisionBspTraverse] Shutdown complete");
}

void LogStats() {
    if (!g_installed) return;
    Log("[CollisionBspTraverse] Calls: %llu, Leaves visited: %llu, Stack fallbacks: %llu%s",
